<section>
    <div class="hero-bg bg-cover w-full h-screen relative">

        <div id="hero-carousel" class="dark-overlay absolute -z-10 top-0 left-0 h-full w-full overflow-hidden">
            <img src="{{ asset('/assets/img/hero/Principal-1.jpg') }}" alt="Hero Background Image">
            <img src="{{ asset('/assets/img/hero/Principal-2.jpg') }}" alt="Hero Background Image">
            <img src="{{ asset('/assets/img/hero/Principal-3.jpg') }}" alt="Hero Background Image">
            <img src="{{ asset('/assets/img/hero/Principal-4.jpg') }}" alt="Hero Background Image">
            <img src="{{ asset('/assets/img/hero/Principal-5.jpg') }}" alt="Hero Background Image">
        </div>

        <div class="text-white text-center flex flex-col justify-center gap-8 border h-full">
            <img class="w-64 mx-auto" src="{{ asset('assets/log-alt.png') }}" alt="logo alt">
            <h2 class="text-2xl font-bold px-12">
                UNA EXCELENTE UBICACIÓN Y CÓMODA ESTADÍA
            </h2>
            <p class="text-4xl font-bold px-10">
                Para tus viajes de placer y negocios
            </p>
        </div>
    </div>
</section>