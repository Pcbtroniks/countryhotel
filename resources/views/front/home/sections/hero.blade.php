<section>
    <div class="hero-bg bg-cover w-full h-screen relative">

        <div id="hero-carousel" class="owl-carousel dark-overlay absolute -z-10 top-0 left-0 h-full w-full overflow-hidden">
                <img class="w-full" src="{{ asset('/assets/img/hero/Principal-1.jpg') }}" alt="Hero Background Image">
                <img class="w-full" src="{{ asset('/assets/img/hero/Principal-2.jpg') }}" alt="Hero Background Image">
                <img class="w-full" src="{{ asset('/assets/img/hero/Principal-3.jpg') }}" alt="Hero Background Image">
                <img class="w-full" src="{{ asset('/assets/img/hero/Principal-4.jpg') }}" alt="Hero Background Image">
                <img class="w-full" src="{{ asset('/assets/img/hero/Principal-5.jpg') }}" alt="Hero Background Image">
        </div>

        <div class="text-white text-center flex flex-col justify-center gap-8 h-full z-10 update absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <img class="w-64 mx-auto" src="{{ asset('assets/log-alt.png') }}" alt="logo alt">
            <h2 class="text-2xl font-bold px-12">
                UNA EXCELENTE UBICACIÓN Y CÓMODA ESTADÍA
            </h2>
            <p class="text-4xl font-bold px-10">
                Para tus viajes de placer y negocios
            </p>
        </div>
    </div>
</section>